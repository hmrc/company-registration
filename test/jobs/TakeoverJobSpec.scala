...
class TakeoverJobSpec extends WordSpec with Matchers with MockitoSugar with LogCapturing with Eventually {
  ...
  "updateTakeoverData" should {
    ...
    "only update the records with an invalid takeover block" in {
      ...
      await(appStartupJobs.updateTakeoverData(regIds)) shouldBe expectedResult
