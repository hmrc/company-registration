...
class ThrottleMongoRepositoryISpec extends IntegrationSpecBase {
  ...
  class Setup {
    val service: ThrottleService = app.injector.instanceOf[ThrottleService]
    val repository: ThrottleMongoRepository = app.injector.instanceOf[ThrottleMongoRepository]
  ...
  "Throttle repository" must {
    "should be able to get an _id" in new Setup {
      val response: Int = await(repository.update(testKey, 10))
      response mustBe 1
    }

    "get sequences, one after another from 1 to the end" in new Setup {
      val inputs: Range.Inclusive = 1 to 7
      val outputs: IndexedSeq[Int] = inputs map { _ => await(repository.update(testKey, 10)) }
