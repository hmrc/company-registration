...
class CTUTRMongoRepositoryISpec extends IntegrationSpecBase with MongoIntegrationSpec {
  ...
  class Setup {
    lazy val repository: CorporationTaxRegistrationMongoRepository = app.injector.instanceOf[CorporationTaxRegistrationMongoRepository]
  ...
  "CT UTR Encryption" must {
    ...
    "store the plain UTR in encrypted form" in new Setup {
      ...
      val ctUtr: Option[String] = validHeldCorporationTaxRegistration.acknowledgementReferences.get.ctUtr
      ...
      val query: Bson = Filters.equal("confirmationReferences.acknowledgement-reference", ackRef)
