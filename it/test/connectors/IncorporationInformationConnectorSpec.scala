...
class IncorporationInformationConnectorSpec extends IntegrationSpecBase {
  ...
  val connector: IncorporationInformationConnector = app.injector.instanceOf[IncorporationInformationConnector]
  "callBackUrl" must {
  ...
  "registerInterest" must {
    "return future successful true if admin is false" in {
      ...
      await(connector.registerInterest("1", "123", admin = false)(implicitly[HeaderCarrier], FakeRequest.apply(Call("", ""))))
    }
      ...
      stubFor(post(urlEqualTo("/incorporation-information/subscribe/123/regime/ctax/subscriber/SCRS?force=true"))
        ...
        .willReturn(
          aResponse().
            withStatus(202).
            withBody("""{ "f" :"c" }""")))
      await(connector.registerInterest("1", "123", admin = true)(implicitly[HeaderCarrier], FakeRequest.apply(Call("", ""))))
    }
      ...
      stubFor(post(urlEqualTo("/incorporation-information/subscribe/123/regime/ctax/subscriber/SCRS?force=true"))
        .willReturn(
          aResponse().
            withStatus(500).
            withBody("""{ "f" :"c" }""")))
      intercept[Exception](await(connector.registerInterest("1", "123", admin = true)(implicitly[HeaderCarrier], FakeRequest.apply(Call("", "")))))
    }
  }
}