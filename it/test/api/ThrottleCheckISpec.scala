...
import test.itutil.WiremockHelper.{stubAuthorise, stubGet, stubPatch, stubPost}
...
class ThrottleCheckISpec extends IntegrationSpecBase with MongoIntegrationSpec with LoginStub {
  ...
  class Setup {
    val crRepo: CorporationTaxRegistrationMongoRepository = app.injector.instanceOf[CorporationTaxRegistrationMongoRepository]
    crRepo.deleteAll
    await(crRepo.ensureIndexes)

    val throttleRepo: ThrottleMongoRepository = app.injector.instanceOf[ThrottleMongoRepository]
